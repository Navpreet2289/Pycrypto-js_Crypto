...
class EncryptHandler(BaseHTTPRequestHandler):
	...
	def _return_file(self, in_file):
		''' '''
	        in_file = '/home/navpreetsingh/crypt/' +in_file
	...
	def do_POST(self):
		...
		if self.path == "/encrypt":
			...
			if postvars:
				...
				iv = "375ac76f53ed0baf3a3f3ae41fefdfb5"
				iv, encrypted = Cryptor.encrypt(to_encrypt, Cryptor.KEY, iv)
			...
			if postvars:
				to_encrypt = ''.join(postvars["to_encrypt"])
				iv = "375ac76f53ed0baf3a3f3ae41fefdfb5"
				encrypted = Cryptor.decrypt(to_encrypt, Cryptor.KEY, iv)
				
				result = {
					"key": Cryptor.KEY,
					"iv": iv,
					"ciphertext": encrypted.rstrip()
				}
